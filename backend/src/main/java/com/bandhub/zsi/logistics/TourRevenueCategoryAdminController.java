...
@RestController
@RequestMapping("/api/admin/tour-revenue-categories")
@PreAuthorize("hasRole('ADMIN')")
class TourRevenueCategoryAdminController {
    private final TourRevenueCategoryAdminService service;
    TourRevenueCategoryAdminController(TourRevenueCategoryAdminService service) { this.service = service; }
    ...
    @PostMapping ResponseEntity<Void> create(@RequestBody @Valid CreateTourCategoryRequest request) { UUID id = service.create(request); return ResponseEntity.created(URI.create("/api/admin/tour-revenue-categories/" + id)).build(); }
