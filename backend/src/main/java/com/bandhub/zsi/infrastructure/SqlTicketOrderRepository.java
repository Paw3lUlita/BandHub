...
class SqlTicketOrderRepository implements TicketOrderRepository {
    ...
    public PagedResult<TicketOrder> findPage(int page, int size, String sortBy, String sortDir, String q, UUID concertId) {
        ...
        var result = jpaRepository.findAllFiltered(pattern, concertId, pageable);
...
interface JpaTicketOrderRepository extends JpaRepository<TicketOrder, UUID> {
    ...
    @Query("""
            ...
            WHERE (LOWER(COALESCE(t.userId, '')) LIKE LOWER(:pattern) OR LOWER(COALESCE(t.status, '')) LIKE LOWER(:pattern))
            AND (:concertId IS NULL OR t.concert.id = :concertId)
            """)
    org.springframework.data.domain.Page<TicketOrder> findAllFiltered(
