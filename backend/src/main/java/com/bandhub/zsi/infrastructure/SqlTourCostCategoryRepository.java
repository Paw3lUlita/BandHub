...
class SqlTourCostCategoryRepository implements TourCostCategoryRepository {
    ...
    @Override
    public PagedResult<TourCostCategory> findPage(int page, int size, String sortBy, String sortDir, String q) {
...
interface JpaTourCostCategoryRepository extends JpaRepository<TourCostCategory, UUID> {
    ...
    @Query("SELECT t FROM TourCostCategory t WHERE LOWER(COALESCE(t.code, '')) LIKE LOWER(:pattern) OR LOWER(COALESCE(t.name, '')) LIKE LOWER(:pattern)")
    org.springframework.data.domain.Page<TourCostCategory> findAllFiltered(@Param("pattern") String pattern, Pageable pageable);
}
