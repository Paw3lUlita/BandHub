...
class CategoryAdminController {
    ...
    @PostMapping
    ResponseEntity<Void> create(@RequestBody @Valid CreateCategoryRequest request) {
        UUID id = service.createCategory(request);
        return ResponseEntity.created(URI.create("/api/admin/categories/" + id)).build();
    }

    @PutMapping("/{id}")
    ResponseEntity<Void> update(@PathVariable UUID id, @RequestBody @Valid CreateCategoryRequest request) {
