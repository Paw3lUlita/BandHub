...
import { TourCostCategoryService } from '../../../core/services/tour-cost-category.service';
...
@Component({
  selector: 'app-tour-cost-category-list',
  standalone: true,
  imports: [AsyncPipe, RouterLink, ListPageControlsComponent],
  template: `
    <div class="overflow-x-auto">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl font-bold">Kategorie Kosztów Trasy</h2>
        <a routerLink="/admin/tour-cost-categories/new" class="btn btn-primary btn-sm">+ Dodaj</a>
      </div>
        ...
        <tbody>
          @for (item of (pageData$ | async)?.content ?? []; track item.id) {
            <tr class="hover">
              ...
              <td class="flex gap-2">
                <a [routerLink]="['/admin/tour-cost-categories', item.id]" class="btn btn-ghost btn-xs">Edytuj</a>
...
export class TourCostCategoryListComponent {
