...
@Component({
  selector: 'app-ticket-refund-list',
  standalone: true,
  imports: [AsyncPipe, CurrencyPipe, DatePipe, RouterLink, ListPageControlsComponent],
  template: `
    <div class="overflow-x-auto">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl font-bold">Zwroty Biletów</h2>
        <a routerLink="/admin/ticket-refunds/new" class="btn btn-primary btn-sm">+ Dodaj</a>
      </div>
        ...
        <tbody>
          @for (item of (pageData$ | async)?.content ?? []; track item.id) {
            <tr class="hover">
              ...
              <td class="flex gap-2">
                <a [routerLink]="['/admin/ticket-refunds', item.id]" class="btn btn-ghost btn-xs">Edytuj</a>
...
export class TicketRefundListComponent {
  private service = inject(TicketRefundService);
  sortOptions = SORT_OPTIONS;

  params = signal<ListPageParams>({ page: 0, size: 10, sortBy: 'requestedAt', sortDir: 'desc', q: '' });
