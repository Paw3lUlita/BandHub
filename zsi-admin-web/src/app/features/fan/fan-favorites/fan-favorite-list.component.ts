...
import { FanFavoriteService } from '../../../core/services/fan-favorite.service';
...
const SORT_OPTIONS: SortOption[] = [
  ...
  { value: 'favoriteType', label: 'Typ' },
  { value: 'referenceId', label: 'Referencja' }
];

@Component({
  selector: 'app-fan-favorite-list',
  standalone: true,
  imports: [AsyncPipe, DatePipe, RouterLink, ListPageControlsComponent],
  template: `
    <div class="overflow-x-auto">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl font-bold">Ulubione Fanów</h2>
        <a routerLink="/admin/fan-favorites/new" class="btn btn-primary btn-sm">+ Dodaj</a>
      </div>
        ...
        <tbody>
          @for (item of (pageData$ | async)?.content ?? []; track item.id) {
            <tr class="hover">
              ...
              <td><span class="badge badge-ghost badge-sm">{{ item.favoriteType }}</span></td>
              <td class="text-xs font-mono">{{ item.referenceId }}</td>
              <td class="text-sm">{{ item.createdAt | date:'short' }}</td>
              <td class="flex gap-2">
                <a [routerLink]="['/admin/fan-favorites', item.id]" class="btn btn-ghost btn-xs">Edytuj</a>
                <button class="btn btn-ghost btn-xs text-error" (click)="onDelete(item.id)">Usuń</button>
              </td>
            </tr>
          } @empty {
            <tr><td colspan="5" class="text-center py-4">Brak ulubionych</td></tr>
          }
        </tbody>
      </table>
    </div>
...
export class FanFavoriteListComponent {
  private service = inject(FanFavoriteService);
