...
import { FanDeviceService } from '../../../core/services/fan-device.service';
...
const SORT_OPTIONS: SortOption[] = [
  ...
  { value: 'lastSeenAt', label: 'Ostatnio widziany' },
  { value: 'fanId', label: 'Fan' },
  { value: 'platform', label: 'Platforma' }
];

@Component({
  selector: 'app-fan-device-list',
  standalone: true,
  imports: [AsyncPipe, DatePipe, RouterLink, ListPageControlsComponent],
  template: `
    <div class="overflow-x-auto">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl font-bold">Urządzenia Fanów</h2>
        <a routerLink="/admin/fan-devices/new" class="btn btn-primary btn-sm">+ Dodaj</a>
      </div>
        ...
        <tbody>
          @for (item of (pageData$ | async)?.content ?? []; track item.id) {
            <tr class="hover">
              ...
              <td><span class="badge badge-ghost badge-sm">{{ item.platform }}</span></td>
              <td>{{ item.appVersion || '-' }}</td>
              <td class="text-sm">{{ item.lastSeenAt | date:'short' }}</td>
              <td class="flex gap-2">
                <a [routerLink]="['/admin/fan-devices', item.id]" class="btn btn-ghost btn-xs">Edytuj</a>
                <button class="btn btn-ghost btn-xs text-error" (click)="onDelete(item.id)">Usuń</button>
              </td>
            </tr>
          } @empty {
            <tr><td colspan="5" class="text-center py-4">Brak urządzeń</td></tr>
          }
        </tbody>
      </table>
    </div>
...
export class FanDeviceListComponent {
  private service = inject(FanDeviceService);
