...
const SORT_OPTIONS: SortOption[] = [
  ...
  { value: 'shippedAt', label: 'Data wysyłki' },
        ...
        <tbody>
          @for (item of (pageData$ | async)?.content ?? []; track item.id) {
            <tr class="hover">
              ...
              <td>{{ item.carrier || '-' }}</td>
              <td class="font-mono text-sm">{{ item.trackingNumber || '-' }}</td>
              <td><span class="badge badge-ghost badge-sm">{{ item.status }}</span></td>
              <td class="text-sm">{{ item.shippedAt | date:'short' }}</td>
              <td class="flex gap-2">
                <a [routerLink]="['/admin/shipments', item.id]" class="btn btn-ghost btn-xs">Edytuj</a>
                <button class="btn btn-ghost btn-xs text-error" (click)="onDelete(item.id)">Usuń</button>
              </td>
            </tr>
          } @empty {
            <tr><td colspan="6" class="text-center py-4">Brak wysyłek</td></tr>
          }
        </tbody>
      </table>
    </div>
